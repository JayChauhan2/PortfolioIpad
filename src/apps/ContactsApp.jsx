...
export default function ContactsApp({ onClose }) {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-gray-50 text-black relative">
      {/* Top Navigation Bar */}
      <div className={`w-full px-6 ${isMobile ? 'pt-6' : 'pt-10'} pb-4 flex justify-between items-center bg-white/80 backdrop-blur-md border-b border-gray-200 sticky top-0 z-10`}>
        <button
      ...
      <div className={`flex-1 w-full flex flex-col ${isMobile ? 'pt-6' : 'pt-10'} pb-16 items-center overflow-y-auto px-4`}>
        <div className={`${isMobile ? 'w-20 h-20' : 'w-24 h-24'} aspect-square flex-shrink-0 rounded-full border-4 border-white shadow-lg overflow-hidden mb-4 bg-gray-100`}>
          <img src="/MyProfile.png" alt="Jay Chauhan" className="w-full h-full object-cover" />
        </div>
        <h2 className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-semibold mb-6`}>Jay</h2>

        <div className="w-full max-w-lg bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden mb-6 text-left">
